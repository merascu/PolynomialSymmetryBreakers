...
** TODO Determine Number of Variables in Breakers from Problem Size [0/3]

If the problem has \(n\) bins, then there will be \(n\) bin variables
\(y_1, \ldots, y_n\) and \(n^2\) selector variables \(x_{11}, \ldots,
x_{1n}, \ldots, x_{n1}, \ldots, x_{nn}\). If \(n\) is too small, then
the (currently) hard-coded number of variables which we select
randomly from the pool of all variables can exceed the number of
available variables. Hence, we will add an option for relative sizes
where the linear polynomials will use \(\lceil p n \rceil\) bin
variables or \(\lceil p n^2 \rceil\) selector variables for some \(p
\in (0,1)\).

For now, the exact value of \(p\) will be hard-coded for each
template. We will also add a minimum size so that we do not end up
with just single variables for the linear polynomials. Similarly, a
maximum size might make sense.


*** TODO Add Command Line Options

We need a command line option parser. For now, it will accept a single
option ~--relative~ (in addition to the LP or SMT2 file name).

We will use {{{OCaml}}}'s built-in ~Arg~ module for now.


*** TODO Add Relative Linear Polynomial Sizes

Find suitable \(p\) for each template.


** TODO Include all Fixed Size Tables and Pick Based on Input Size
...
** TODO More Efficient Generation of Breakers [0/2]
...
*** TODO Implement the Variables Function

Implement an {{{OCaml}}} version of \(\vars\).
...
different names.) Finally, we define an order which has \(x_{i,k} <
y_\ell\) for all \(i,k,\ell\), \(y_k < y_\ell\) iff \(k < \ell\), and
\(x_{i,k} < x_{j,\ell}\) if \((i,k)\) is less than \((j,\ell)\)
lexicographically.
...
type fixed_spec =
  ...
  | Sum of fixed_spec list

let fixed_lin var n = Lin (var, n)
and fixed_sq var n = Sq (var, n)
and fixed_quad var1 n1 var2 n2 = Quad (var1, n1, var2, n2)
and fixed_sum list = Sum list

let rec of_fixed_spec ({xs; ys; m; n} as bins) = function
  ...
  | Sum sps ->
     List.map (of_fixed_spec bins) sps
...
** COMMENT Relative Polynomial Specifications

For relative breaker sizes, we introduce a new specification type
where the single integer values are replaced by a relative size type
with a minimum value \(u \in \NN\), a maximum value \(v \in \NN\), and
a scaling factor \(p \in (0,1)\). Given the number \(n\) of available
variables, we will then randomly choose
\begin{equation*}
  \max \{\min \{ u, \lceil p n \rceil \}, v\}
\end{equation*}
many variables.

In order to make the implementation work more smoothly, we also
redefine the generators for random linear and quadratic polynomials so
that they apply the relative sizes.

Note that in extreme cases we might still request too many
variables. (For example, if the minimum \(v\) is larger than the
number of variables \(n\).) In that case, the program will fail with
an exception.

#+BEGIN_SRC ocaml :tangle no :eval never
type relative_size = {
    minimum : int;
    factor : float;
    maximum : int }

let rel_size minimum factor maximum = {minimum; factor; maximum}

type relative_spec =
  | RelLin of (var * relative_size)
  | RelSq of (var * relative_size)
  | RelQuad of (var * relative_size * var * relative_size)
  | RelSum of relative_spec list

let rel_lin var u p v = RelLin (var, rel_size u p v)
and rel_sq var u p v = RelSq (var, rel_size u p v)
and rel_quad var1 u1 p1 v1 var2 u2 p2 v2 =
  RelQuad (var1, rel_size u1 p1 v1,
           var2, rel_size u2 p2 v2)
and rel_sum specs = RelSum specs

let relative {minimum = u; factor = p; maximum = v} n =
  let pn = ceil (p *. float n) |> int_of_float in
  let r = max (min u pn) v in
  if r > n
  then failwith "Too many variables requested."
  else r

let relative_random_linear ~vars ~spec =
  let len = relative spec (Array.length vars) in
  random_linear ~vars ~len

let relative_random_quadratic ~vars1 ~spec1 ~vars2 ~spec2 =
  let len1 = relative spec1 (Array.length vars1) 
  and len2 = relative spec2 (Array.length vars2)
  in
  random_quadratic ~vars1 ~len1 ~vars2 ~len2

let rec of_relative_spec spec =
  match spec with 
    | RelLin (X, p) -> relative_random_linear ~vars:xs ~spec:p
    | RelLin (Y, p) -> relative_random_linear ~vars:ys ~spec:p
    | RelSq (X, p) ->
       relative_random_quadratic ~vars1:xs ~spec1:p ~vars2:xs ~spec2:p
    | RelSq (Y, p) ->
       relative_random_quadratic ~vars1:ys ~spec1:p ~vars2:ys ~spec2:p
    | RelQuad (X, p, X, q) ->
       relative_random_quadratic ~vars1:xs ~spec1:p ~vars2:xs ~spec2:q
    | RelQuad (X, p, Y, q) ->
       relative_random_quadratic ~vars1:xs ~spec1:p ~vars2:ys ~spec2:q
    | RelQuad (Y, p, X, q) ->
       relative_random_quadratic ~vars1:ys ~spec1:p ~vars2:xs ~spec2:q
    | RelQuad (Y, p, Y, q) ->
       relative_random_quadratic ~vars1:ys ~spec1:p ~vars2:ys ~spec2:q
    | RelSum sps ->
       List.map of_relative_spec sps
       |> List.fold_left Polynomial.add Polynomial.zero
#+END_SRC
...
In order to treat both fixed and relative polynomial specification
cases with the same code, we combine them into a common type.

#+BEGIN_SRC ocaml :tangle no :eval never
type spec = Fixed of fixed_spec | Relative of relative_spec

let of_spec = function
  | Fixed spec -> of_fixed_spec spec
  | Relative spec -> of_relative_spec spec
#+END_SRC


#+BEGIN_SRC ocaml :tangle breakers.ml :eval never
let of_spec = of_fixed_spec
#+END_SRC
...
let run filename =
  ...
  List.iter (run ~sexp ~problem ~symmetries ~prefix:conf.prefix ~repeat)
    (if problem.n < 20 then failwith "Two few variables"
