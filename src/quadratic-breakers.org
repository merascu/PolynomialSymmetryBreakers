...
We start with a small number of variables and permutations.

#+BEGIN_SRC ocaml :tangle breakers.ml :eval never
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "few_vars_few_perms_only_xs"
  and transpositions = all_perms
  and base () =
    random_quadratic ~vars1:xs ~len1:3 ~vars2:xs ~len2:3
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
#+BEGIN_SRC ocaml :tangle breakers.ml :eval never
let () =  
  let permutations = 500
  and repeat = 10
  and descr = "few_vars_many_perms_only_xs"
  and transpositions = all_perms
  and base () =
    random_quadratic ~vars1:xs ~len1:3 ~vars2:xs ~len2:3
  in
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "many_vars_few_perms_only_xs"
  and transpositions = all_perms
  and base () =
    random_quadratic ~vars1:xs ~len1:25 ~vars2:xs ~len2:25
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "thousand_vars_few_perms_only_xs"
  and transpositions = all_perms
  and base () =
    random_quadratic ~vars1:xs ~len1:64 ~vars2:xs ~len2:64
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
Here, we consider purely quadratic polynomials in the \(y\)'s only.
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "few_vars_few_perms_only_ys"
  and transpositions = col_perms
  and base () =
    random_quadratic ~vars1:ys ~len1:3 ~vars2:ys ~len2:3
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
let () =  
  let permutations = 500
  and repeat = 10
  and descr = "few_vars_many_perms_only_ys"
  and transpositions = col_perms
  and base () =
    random_quadratic ~vars1:ys ~len1:3 ~vars2:ys ~len2:3
  in
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "many_vars_few_perms_only_ys"
  and transpositions = col_perms
  and base () =
    random_quadratic ~vars1:ys ~len1:25 ~vars2:ys ~len2:25
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC


A gaggle of variables.

#+BEGIN_SRC ocaml :tangle breakers.ml :eval never
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "thousand_vars_few_perms_only_ys"
  and transpositions = col_perms
  and base () =
    random_quadratic ~vars1:ys ~len1:64 ~vars2:ys ~len2:64
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "few_vars_few_perms_mixed"
  and transpositions = all_perms
  and base () =
    random_quadratic ~vars1:xs ~len1:3 ~vars2:ys ~len2:3
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
let () =  
  let permutations = 500
  and repeat = 10
  and descr = "few_vars_many_perms_mixed"
  and transpositions = all_perms
  and base () =
    random_quadratic ~vars1:xs ~len1:3 ~vars2:ys ~len2:3
  in
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "many_vars_few_perms_mixed"
  and transpositions = all_perms
  and base () =
    random_quadratic ~vars1:xs ~len1:25 ~vars2:ys ~len2:25
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "thousand_vars_few_perms_mixed"
  and transpositions = all_perms
  and base () =
    random_quadratic ~vars1:xs ~len1:64 ~vars2:xs ~len2:64
  in
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "few_vars_few_perms_separate"
  and transpositions = all_perms
  and base () =
    let a = random_quadratic ~vars1:xs ~len1:3 ~vars2:xs ~len2:3
    and b = random_quadratic ~vars1:ys ~len1:3 ~vars2:ys ~len2:3
    in
    Polynomial.(a + b)
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
...
let () =  
  let permutations = 500
  and repeat = 10
  and descr = "few_vars_many_perms_separate"
  and transpositions = all_perms
  and base () =
    let a = random_quadratic ~vars1:xs ~len1:3 ~vars2:xs ~len2:3
    and b = random_quadratic ~vars1:ys ~len1:3 ~vars2:ys ~len2:3
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "many_vars_few_perms_separate"
  and transpositions = all_perms
  and base () =
    let a = random_quadratic ~vars1:xs ~len1:20 ~vars2:xs ~len2:20
    and b = random_quadratic ~vars1:ys ~len1:20 ~vars2:ys ~len2:20
    in
    Polynomial.(a + b)
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "thousand_vars_few_perms_separate"
  and transpositions = all_perms
  and base () =
    let a = random_quadratic ~vars1:xs ~len1:64 ~vars2:xs ~len2:64
    and b = random_quadratic ~vars1:ys ~len1:64 ~vars2:ys ~len2:64
    in
    Polynomial.(a + b)
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "few_vars_few_perms_linear_x"
  and transpositions = all_perms
  and exclude q = Polynomial.(is_zero q || is_linear q)
  and base () =
    let a = random_linear ~vars:xs ~len:7 
    and b = random_quadratic ~vars1:ys ~len1:3 ~vars2:ys ~len2:3
    in
    Polynomial.(a + b)
  in
  Experiment.run ~exclude ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
let () =  
  let permutations = 500
  and repeat = 10
  and descr = "few_vars_many_perms_linear_x"
  and transpositions = all_perms
  and exclude q = Polynomial.(is_zero q || is_linear q)
  and base () =
    let a = random_linear ~vars:xs ~len:7 
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "many_vars_few_perms_linear_x"
  and transpositions = all_perms
  and exclude q = Polynomial.(is_zero q || is_linear q)
  and base () =
    let a = random_linear ~vars:xs ~len:70 
    and b = random_quadratic ~vars1:ys ~len1:20 ~vars2:ys ~len2:20
    in
    Polynomial.(a + b)
  in
...
Really many more variables.

#+BEGIN_SRC ocaml :tangle breakers.ml :eval never
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "thousand_vars_few_perms_linear_x"
  and transpositions = all_perms
  and exclude q = Polynomial.(is_zero q || is_linear q)
  and base () =
    let a = random_linear ~vars:xs ~len:(min 2500 (m * n))
    and b = random_quadratic ~vars1:ys ~len1:50 ~vars2:ys ~len2:50
    in
    Polynomial.(a + b)
  in
  Experiment.run ~exclude ~prefix ~transpositions ~permutations ~descr ~repeat base
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "few_vars_few_perms_linear_y"
  and transpositions = all_perms
  and exclude q = Polynomial.(is_zero q || is_linear q)
  and base () =
    let a = random_linear ~vars:ys ~len:7 
    and b = random_quadratic ~vars1:xs ~len1:3 ~vars2:xs ~len2:3
    in
    Polynomial.(a + b)
  in
  Experiment.run ~exclude ~prefix ~transpositions ~permutations ~descr ~repeat base
...
let () =  
  let permutations = 500
  and repeat = 10
  and descr = "few_vars_many_perms_linear_y"
  and transpositions = all_perms
  and exclude q = Polynomial.(is_zero q || is_linear q)
  and base () =
    let a = random_linear ~vars:ys ~len:7 
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "many_vars_few_perms_linear_y"
  and transpositions = all_perms
  and exclude q = Polynomial.(is_zero q || is_linear q)
  and base () =
    let a = random_linear ~vars:ys ~len:70 
    and b = random_quadratic ~vars1:xs ~len1:20 ~vars2:xs ~len2:20
    in
    Polynomial.(a + b)
  in
  Experiment.run ~exclude ~prefix ~transpositions ~permutations ~descr ~repeat base
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "thousand_vars_few_perms_linear_y"
  and transpositions = all_perms
  and exclude q = Polynomial.(is_zero q || is_linear q)
  and base () =
    let a = random_linear ~vars:ys ~len:(min 2500 n)
    and b = random_quadratic ~vars1:xs ~len1:50 ~vars2:xs ~len2:50
    in
    Polynomial.(a + b)
  in
  Experiment.run ~exclude ~prefix ~transpositions ~permutations ~descr ~repeat base
...
** Linear Polynomials in \(x\)

Purely linear polynomials using only \(x\)'s.

#+BEGIN_SRC ocaml :tangle breakers.ml :eval never
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "few_vars_few_perms_pure_linear_only_x"
  and transpositions = all_perms
  and base () = random_linear ~vars:xs ~len:10
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
let () =  
  let permutations = 500
  and repeat = 10
  and descr = "few_vars_many_perms_pure_linear_only_x"
  and transpositions = all_perms
  and base () = random_linear ~vars:xs ~len:10
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "many_vars_few_perms_pure_linear_only_x"
  and transpositions = all_perms
  and base () = random_linear ~vars:xs ~len:1_000
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "thousand_vars_few_perms_pure_linear_only_x"
  and transpositions = all_perms
  and base () = random_linear ~vars:xs ~len:4_000
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
** Linear Polynomials in \(y\)

Purely linear polynomials using only \(y\)'s.

#+BEGIN_SRC ocaml :tangle breakers.ml :eval never
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "few_vars_few_perms_pure_linear_only_y"
  and transpositions = all_perms
  and base () = random_linear ~vars:ys ~len:10
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
let () =  
  let permutations = 500
  and repeat = 10
  and descr = "few_vars_many_perms_pure_linear_only_y"
  and transpositions = all_perms
  and base () = random_linear ~vars:ys ~len:10
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "many_vars_few_perms_pure_linear_only_y"
  and transpositions = all_perms
  and base () = random_linear ~vars:ys ~len:1_000
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "thousand_vars_few_perms_pure_linear_only_y"
  and transpositions = all_perms
  and base () = random_linear ~vars:ys ~len:4_000
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
#+END_SRC
...
** Linear Polynomials in \(x\) and \(y\)

Purely linear polynomials using \(x\)'s and \(y\)'s.

#+BEGIN_SRC ocaml :tangle breakers.ml :eval never
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "few_vars_few_perms_pure_linear_x_and_y"
  and transpositions = all_perms
  and base () =
    let a = random_linear ~vars:xs ~len:5
    and b = random_linear ~vars:ys ~len:5
    in
    Polynomial.(a + b)
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
...
let () =  
  let permutations = 500
  and repeat = 10
  and descr = "few_vars_many_perms_pure_linear_x_and_y"
  and transpositions = all_perms
  and base () =
    let a = random_linear ~vars:xs ~len:5
    and b = random_linear ~vars:ys ~len:5
...
For the larger classes, we take more \(x\)'s than \(y\)'s simply
because we do not have so many \(y\)'s.

#+BEGIN_SRC ocaml :tangle breakers.ml :eval never
let () =  
  let permutations = 5
  and repeat = 10
  and descr = "many_vars_few_perms_pure_linear_x_and_y"
  and transpositions = all_perms
  and base () =
    let a = random_linear ~vars:xs ~len:750
    and b = random_linear ~vars:ys ~len:250
    in
    Polynomial.(a + b)
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
...
let () =  
  let permutations = 50
  and repeat = 10
  and descr = "thousand_vars_few_perms_pure_linear_x_and_y"
  and transpositions = all_perms
  and base () =
    let a = random_linear ~vars:xs ~len:3_000
    and b = random_linear ~vars:ys ~len:1_000
    in
    Polynomial.(a + b)
  in
  Experiment.run ~prefix ~transpositions ~permutations ~descr ~repeat base
